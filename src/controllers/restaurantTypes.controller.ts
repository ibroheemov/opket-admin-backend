...
import { RestaurantTypeModel } from "../models/RestaurantType";


/** GET /food/categories */
export const listRestaurantTypes = async (req: Request, res: Response) => {
    const categories = await RestaurantTypeModel.find()
        .sort({ sort_order: 1 })
        .lean();
...
export const createRestaurantType = async (req: Request, res: Response) => {
    ...
    const category = await RestaurantTypeModel.create({
        name,
        sort_order,
        image_url,
        image_public_id,
...
export const updateRestaurantType = async (req: Request, res: Response) => {
    const { name, sort_order } = req.body;

    const existing = await RestaurantTypeModel.findById(req.params.id);
    if (!existing) {
...
export const deleteRestaurantType = async (req: Request, res: Response) => {
    const existing = await RestaurantTypeModel.findById(req.params.id);
    if (!existing) {
